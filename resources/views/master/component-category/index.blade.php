@extends('layouts.app')
@section('title', 'Kategori Komponen')
@section('page-title', 'Component Category')
...
<div x-data="{ 
    ...
    validateCurrentForm() {
        if (!(this.formData.name || '').trim()) {
            window.toast.error('Nama kategori wajib diisi.');
            return false;
        }
    ...
    async submitForm() {
        ...
        if (this.editMode) {
            ...
            try {
                const response = await axios.put(`{{ route('master.component-category.index') }}/${this.formData.id}`, {
                    name: this.formData.name,
                });

                window.toast.success(response.data.message || 'Kategori komponen berhasil diperbarui.');
                this.showModal = false;
        ...
        try {
            for (const item of payloads) {
                await axios.post(`{{ route('master.component-category.store') }}`, {
                    name: item.name,
                });
                successCount += 1;
            }
            ...
            this.pendingItems = [];
            this.resetCreateForm();
            window.toast.success(`${successCount} kategori komponen berhasil disimpan.`);
            this.showModal = false;
            setTimeout(() => window.location.reload(), 500);
        } catch (error) {
...
}" x-init="@if(session('reopen_component_category_create')) openCreate() @endif">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="page-title text-2xl font-bold text-slate-900">Kategori Komponen</h1>
            <p class="text-sm text-slate-500 mt-1">Kelola kategori komponen untuk kebutuhan BOM dan master komponen.</p>
        </div>
        
        @if(\App\Helpers\MenuHelper::hasPermission('component-category', 'create'))
    ...
    <div class="card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table-base">
                ...
                <tbody>
                    @forelse($componentCategories as $item)
                    <tr>
                        <td><span class="text-slate-400 font-mono text-xs">{{ ($componentCategories->currentPage() - 1) * $componentCategories->perPage() + $loop->iteration }}</span></td>
                        <td class="font-bold text-slate-700">{{ $item['name'] }}</td>
                        <td>
                            <div class="flex items-center justify-center gap-1">
                                @if(\App\Helpers\MenuHelper::hasPermission('component-category', 'detail'))
                                ...
                                @if(\App\Helpers\MenuHelper::hasPermission('component-category', 'edit'))
                                ...
                                @if(\App\Helpers\MenuHelper::hasPermission('component-category', 'delete'))
                                <button @click="confirmModal.confirm('{{ route('master.component-category.destroy', $item['id']) }}')" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Hapus"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                @endif
                            </div>
                        </td>
                    </tr>
        ...
        @if($componentCategories->hasPages())
        <div class="p-4 border-t border-slate-100 bg-slate-50/30">
            {{ $componentCategories->links() }}
        </div>
        @endif
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center">
            ...
            <div class="relative inline-block w-full max-w-md p-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-3xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-slate-900" x-text="detailMode ? 'Detail Kategori Komponen' : (editMode ? 'Edit Kategori Komponen' : 'Tambah Kategori Komponen')"></h3>
                ...
                <form @submit.prevent="submitForm()" novalidate>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" x-model="formData.name" class="form-input w-full" placeholder="Contoh: Bearing, Motor, Rangka" required>
                        </div>
                    </div>
                    </fieldset>
