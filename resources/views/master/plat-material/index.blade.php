@extends('layouts.app')
@section('title', 'Jenis Material')
@section('page-title', 'Plat Material')
...
<div x-data="{ 
    ...
    validateCurrentForm() {
        if (!(this.formData.name || '').trim()) {
            window.toast.error('Nama material wajib diisi.');
            return false;
        }
    ...
    async submitForm() {
        ...
        if (this.editMode) {
            ...
            try {
                const response = await axios.put(`{{ route('master.plat-material.index') }}/${this.formData.id}`, {
                    name: this.formData.name,
                });

                window.toast.success(response.data.message || 'Jenis material berhasil diperbarui.');
                this.showModal = false;
        ...
        try {
            for (const item of payloads) {
                await axios.post(`{{ route('master.plat-material.store') }}`, {
                    name: item.name,
                });
                successCount += 1;
            }
            ...
            this.pendingItems = [];
            this.resetCreateForm();
            window.toast.success(`${successCount} material berhasil disimpan.`);
            this.showModal = false;
            setTimeout(() => window.location.reload(), 500);
        } catch (error) {
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="page-title text-2xl font-bold text-slate-900">Jenis Material Plat</h1>
            <p class="text-sm text-slate-500 mt-1">Kelola kategori material yang tersedia (Besi, Stainless, dll).</p>
        </div>
        
        @if(\App\Helpers\MenuHelper::hasPermission('plate-material', 'create'))
        ...
        <div class="overflow-x-auto">
            <table class="table-base">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="text-slate-400 font-mono text-xs">{{ ($materials->currentPage() - 1) * $materials->perPage() + $loop->iteration }}</span></td>
                        <td class="font-bold text-slate-700">{{ $item['name'] }}</td>
                        <td>
                            <div class="flex items-center justify-center gap-1">
                                @if(\App\Helpers\MenuHelper::hasPermission('plate-material', 'detail'))
                                ...
                                @if(\App\Helpers\MenuHelper::hasPermission('plate-material', 'edit'))
                                ...
                                @if(\App\Helpers\MenuHelper::hasPermission('plate-material', 'delete'))
                                <button @click="confirmModal.confirm('{{ route('master.plat-material.destroy', $item['id']) }}')" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Hapus"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                @endif
                            </div>
                        </td>
                    </tr>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center">
            ...
            <div class="relative inline-block w-full max-w-md p-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-3xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-slate-900" x-text="detailMode ? 'Detail Material' : (editMode ? 'Edit Material' : 'Tambah Material')"></h3>
                ...
                <form @submit.prevent="submitForm()" novalidate>
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Nama Material</label>
                            <input type="text" name="name" x-model="formData.name" class="form-input w-full" placeholder="Contoh: Besi Hitam, Stainless 304" required>
                        </div>
                    </div>
                    </fieldset>
